...
<div className="team-switch"><span className="team-avatar">4S</span><div><strong>{syncResult ? "Roster synchronisé" : "Aucun roster"}</strong><small>{syncResult ? `${syncResult.roster.length} joueurs Riot` : "Ajoutez votre équipe"}</small></div><span className="chevron">⌄</span></div>
        <nav><button type="button" className={section === "team" ? "active" : ""} onClick={() => setSection("team")}><span>◫</span>Vue d’équipe</button><button type="button" className={section === "playbook" ? "active" : ""} onClick={() => setSection("playbook")}><span>≡</span>Playbook</button><button type="button" className={section === "coaching" ? "active" : ""} onClick={() => setSection("coaching")}><span>◇</span>Coaching</button><button type="button" className={section === "review" ? "active" : ""} onClick={() => setSection("review")}><span>✦</span>Review</button></nav>
