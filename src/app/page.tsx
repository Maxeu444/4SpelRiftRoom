...
function DuoDashboard({ analysis, roleAssignments, onSync }: { analysis: TeamAnalysis | null; roleAssignments: Record<string, Role>; onSync: () => void }) {
  ...
  return <section className="duo-view">
    ...
    <section className="panel duo-pairs"><div className="panel-head"><div><p className="eyebrow">Paires réellement jouées</p><h2>Quel duo fonctionne, et où ?</h2></div><span className="pill">{duo.pairs.length} duo{duo.pairs.length > 1 ? "s" : ""}</span></div><p className="duo-pairs-intro">Les indicateurs sont adaptés à la lane la plus souvent jouée par chaque paire, pour comparer leur coordination avec des repères utiles à leurs rôles.</p><div className="duo-pair-grid">{duo.pairs.map((pair) => {
      const contextualMetrics = pair.contextualMetrics ?? [];
      return <article key={pair.playerPuuids.join(":")}><div className="duo-pair-members"><span>{pair.playerNames[0].slice(0, 1)}</span><i>+</i><span>{pair.playerNames[1].slice(0, 1)}</span></div><h3>{pair.playerNames.join(" · ")}</h3><p className="duo-pair-lane">{pair.lane ?? "Lane à actualiser"}</p><small className="duo-pair-sample">{pair.games} game{pair.games > 1 ? "s" : ""} · {pair.averageDurationMinutes} min de moyenne</small><div className="duo-pair-score"><strong>{pair.winRate} % WR</strong><small>{pair.wins} victoire{pair.wins > 1 ? "s" : ""} · {pair.combinedKda ?? "—"} KDA</small></div>{contextualMetrics.length ? <div className="duo-context-metrics">{contextualMetrics.map((metric) => <div key={metric.label}><span>{metric.label}</span><strong>{metric.value}{metric.unit}</strong><small>{metric.detail}</small></div>)}</div> : <p className="duo-context-empty">Synchronisez à nouveau pour obtenir les statistiques adaptées à cette lane.</p>}<div className="duo-pair-footer"><span>{pair.killParticipation ?? "—"}{pair.killParticipation === null || pair.killParticipation === undefined ? "" : " %"} implication kills</span><span>{pair.visionPerMinute ?? "—"} vision/min</span></div></article>;
    })}</div></section>
